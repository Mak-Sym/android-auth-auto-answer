...
public class AuthenticatorAutoAnswerIntentService extends IntentService {
	...
	public AuthenticatorAutoAnswerIntentService() {
		this("PhonePhactorAutoAnswerIntentService");
	}
	
	public AuthenticatorAutoAnswerIntentService(String name) {
		super("PhonePhactorAutoAnswerIntentService");
	}
	...
	protected void onHandleIntent(Intent intent) {
		Log.d("PhonePhactorAutoAnswerIntentService.onReceive", "Received!");
		try {
	        ...
	        if (tm.getCallState() != TelephonyManager.CALL_STATE_RINGING) {
	        	Log.d("PhonePhactorAutoAnswerIntentService.answerToPhonePhactor", "not ringing any more :(");
	        	return;
	        }
	        Log.d("PhonePhactorAutoAnswerIntentService.answerToPhonePhactor", "Still Ringing!");
			...
			e.printStackTrace();
            Log.w("PhonePhactorAutoAnswerIntentService.onReceive","Error trying to answer using telephony service. Intent: " + intent);
		}
		return;
	}
	...
	protected void pickUp(Context context, ITelephony telephonyService) throws Exception {
		try {
			 ...
			 */
			Log.d("PhonePhactorAutoAnswerIntentService.pickUp", "Silence Ringer using ITelephony");
			telephonyService.silenceRinger();
			Log.d("PhonePhactorAutoAnswerIntentService.pickUp", "Answer the call using ITelephony");
			telephonyService.answerRingingCall();
		} catch (Exception e){
			e.printStackTrace();
			Log.d("PhonePhactorAutoAnswerService.pickUp", "Error pickimng up the phone using ITelephony", e);
			
			// Simulate a press of the headset button to pick up the call
			Log.d("PhonePhactorAutoAnswerIntentService.pickUp", "Intent.ACTION_MEDIA_BUTTON Down");
			Intent buttonDown = new Intent(Intent.ACTION_MEDIA_BUTTON);		
			buttonDown.putExtra(Intent.EXTRA_KEY_EVENT, new KeyEvent(KeyEvent.ACTION_DOWN, KeyEvent.KEYCODE_HEADSETHOOK));
			context.sendOrderedBroadcast(buttonDown, "android.permission.CALL_PRIVILEGED");

			// froyo and beyond trigger on buttonUp instead of buttonDown
			Log.d("PhonePhactorAutoAnswerIntentService.pickUp", "Intent.ACTION_MEDIA_BUTTON Up");
	...
	protected void answer(Context context) {
		//wait
        Log.d("PhonePhactorAutoAnswerIntentService.answer", "Waiting for 1000ms");
        ...
        Log.d("PhonePhactorAutoAnswerIntentService.answer", "generating tone");
	...
	protected void hangUp(Context context, ITelephony telephonyService) throws Exception {
		Log.d("PhonePhactorAutoAnswerIntentService.hangUp", "Waiting for 1000ms");
        try { Thread.sleep(1000); }
        catch(Exception ignored){}
		//end call
		Log.d("PhonePhactorAutoAnswerIntentService.answerToPhonePhactor", "End Call");
